...
const StrategyDrawer: React.FC<StrategyDrawerProps> = ({
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-40 bg-black/98 "
        onClick={onClose}
      />
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full flex items-center justify-center border border-white/10 bg-black/60 text-white hover:text-white hover:bg-white/10 transition-all duration-300"
          >
            <X className="w-8 h-8" />
          </button>
        </div>
